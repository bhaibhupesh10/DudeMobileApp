...
import { LOCATION_PRICING } from '../constants/location';
        ...
        <View className="mt-auto bg-white rounded-t-3xl h-[85%]">
          ...
          <ScrollView className="flex-1" bounces={false}>
            ...
            <View className="p-4">
              ...
              {/* Available Locations */}
              <View className="mt-6">
                <Text className="font-bold mb-3">Available Delivery Locations</Text>
                {Object.entries(LOCATION_PRICING).map(([pincode, data]) => (
                  <TouchableOpacity
                    key={pincode}
                    className={`p-4 border rounded-lg mb-2 ${
                      currentPincode === pincode ? 'border-green-500' : 'border-gray-200'
                    }`}
                    onPress={async () => {
                      await setManualPincode(pincode as any);
                      onClose();
                    }}
                  >
                    <Text className="font-medium">{data.areaName}</Text>
                    <Text className="text-sm text-gray-500">Pincode: {pincode}</Text>
                    <Text className="text-xs text-green-600">
                      {data.priceMultiplier !== 1 
                        ? data.priceMultiplier > 1 
                          ? `Delivery charges apply (${((data.priceMultiplier - 1) * 100).toFixed(0)}% extra)`
                          : `Discounted delivery available (${((1 - data.priceMultiplier) * 100).toFixed(0)}% off)`
          ...
          <View className="p-4 border-t border-gray-200">
            ...
            >
              <Text className="text-white text-center font-semibold">
                {locationLoading ? 'Updating Location...' : 'Confirm Location'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
