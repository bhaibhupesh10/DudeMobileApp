...
import { signOut } from '../../lib/handleAuth';
import { router } from 'expo-router';
import { MENU_ITEMS, MenuItem as MenuItemType } from '../../constants/profile';
...
const ProfileScreen: React.FC = () => {
  ...
  const handleMenuPress = (menuItem: MenuItemType) => {
    switch (menuItem.id) {
      case 'logout':
        handleLogout();
        break;
  ...
  const MenuItemComponent: React.FC<MenuItemProps> = ({ icon, title, onPress }) => (
    <TouchableOpacity
      onPress={onPress}
      className="flex-row items-center px-4 py-3 border-b border-gray-100"
      activeOpacity={0.7}
